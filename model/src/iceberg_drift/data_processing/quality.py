        ...
        if method == "nearest_valid":
            ...
            for t in range(data.shape[0]):
                for d in range(data.shape[1]):
                    slice_data = data[t, d, :, :]
                    slice_nans = np.isnan(slice_data)

                    if slice_nans.all() or not slice_nans.any():
                        continue
                    ...
                    valid_mask = ~slice_nans
                    invalid_mask = slice_nans

                    valid_lats = lat_idx[valid_mask]
                    valid_lons = lon_idx[valid_mask]
                    valid_vals = slice_data[valid_mask]

                    invalid_lats = lat_idx[invalid_mask]
                    invalid_lons = lon_idx[invalid_mask]

                    # Interpolate
                    filled = griddata(
                        np.column_stack([valid_lats, valid_lons]),
                        valid_vals,
                        np.column_stack([invalid_lats, invalid_lons]),
                        method="nearest",
                    )

                    slice_data[invalid_mask] = filled
                    data[t, d, :, :] = slice_data
