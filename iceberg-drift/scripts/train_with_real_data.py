...
def load_real_iceberg_data(data_dir: str) -> pd.DataFrame:
    ...
    if not all_dfs:
        try:
            from iceberg_drift.data_processing.download import _load_byu_local_icebergs
            logger.info(f"Using BYU loader fallback for {data_dir}")
            combined = _load_byu_local_icebergs(data_dir, "2010-01-01", "2024-12-31", 100)
            # Add orig_iceberg_id if missing (expected by standardizer, though we might bypass it)
            if 'orig_iceberg_id' not in combined.columns:
                combined['orig_iceberg_id'] = combined['iceberg_id']
                combined['source'] = 'BYU_LOCAL'
            return combined
        except Exception as e:
            raise ValueError(f"No CSV files found in {data_dir}, fallback failed: {e}")
...
def standardize_iceberg_columns(df: pd.DataFrame) -> pd.DataFrame:
    ...
    # Process each source separately to avoid the pandas trap
    processed_dfs = []

    # Process AAD 1979-1984
    aad_79_84_mask = df['source'] == 'AAD_1979_1984'
    if aad_79_84_mask.any():
        logger.info(f"Applying AAD mapping for AAD_1979_1984, rows: {aad_79_84_mask.sum()}")
        subset = df[aad_79_84_mask].copy()
        subset = subset.rename(columns=aad_mapping)
        processed_dfs.append(subset)

    # Process AAD 1984-2011 (apply the correct mapping for swapped lat/lon)
    aad_84_11_mask = df['source'] == 'AAD_1984_2011'
    if aad_84_11_mask.any():
        logger.info(f"Applying AAD 84-11 mapping for AAD_1984_2011 (swapped lat/lon), rows: {aad_84_11_mask.sum()}")
        subset = df[aad_84_11_mask].copy()
        subset = subset.rename(columns=aad_84_11_mapping)
        processed_dfs.append(subset)

    # Process NPI 1977-2010
    npi_mask = df['source'] == 'NPI_1977_2010'
    if npi_mask.any():
        logger.info(f"Applying NPI mapping for NPI_1977_2010, rows: {npi_mask.sum()}")
        subset = df[npi_mask].copy()
        subset = subset.rename(columns=npi_mapping)
        processed_dfs.append(subset)

    # Handle any other sources (keep as-is)
    other_mask = ~(aad_79_84_mask | aad_84_11_mask | npi_mask)
    if other_mask.any():
        logger.info(f"Keeping other sources as-is, rows: {other_mask.sum()}")
        processed_dfs.append(df[other_mask].copy())
    ...
    if 'datetime' in df.columns:
        # Some legacy datasets have backslash escapes in ISO strings
        datetime_series = df['datetime'].astype(str).str.replace('\\', '', regex=False)
        df['datetime'] = pd.to_datetime(datetime_series, errors='coerce', format='ISO8601')
    else:
        raise ValueError(f"No datetime column found in data after renaming. Columns: {df.columns.tolist()}")
...
def main():
    ...
    parser.add_argument("--data-dir", type=str, default="data/raw/iceberg_positions", help="Directory with CSV files")
    ...
    wind_ds = download_era5_wind(
        ...
        start_date=config.data_sources.iceberg_positions.start_date,
        end_date=config.data_sources.iceberg_positions.end_date,
        bboxes=bboxes,
    ...
    feature_cols = [c for c in train_df.columns if not c.startswith(('target_', 'iceberg_id', 'orig_iceberg_id', 'datetime', 'lat', 'lon', 'source', 'cruise_id', 'vessel', 'rv', 'ice_concentration', 'total_count'))]
