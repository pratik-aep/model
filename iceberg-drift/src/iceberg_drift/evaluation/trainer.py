...
class DriftTrainer:
    ...
    def __init__(
        ...
        # Feature metadata for inference compatibility
        ...
        if feature_idx_map is not None:
            logger.info(f"Feature metadata loaded: {len(feature_idx_map)} features")
    ...
    def train_epoch(self) -> Dict[str, float]:
        ...
        for batch_idx, batch in enumerate(progress_bar):
            ...
            # Check for NaN or Inf loss - skip batch if detected
            if torch.isnan(loss) or torch.isinf(loss):
                logger.warning(f"Batch {batch_idx}: Loss is {loss.item()}, skipping backward pass")
    ...
    def validate(self) -> Dict[str, float]:
        ...
        with torch.no_grad():
            for batch in tqdm(self.val_loader, desc="Validation", leave=False):
                ...
                # Check for NaN or Inf loss - skip batch if detected
                if torch.isnan(loss) or torch.isinf(loss):
                    logger.warning(f"Validation batch {num_batches}: Loss is {loss.item()}, skipping")
                    continue  # Skip to next batch
...
def train_model(
    ...
    Args:
        ...
        feature_idx_map: Mapping of feature names to indices (for inference compatibility)
        feature_scaler: Fitted scaler for feature normalization (for inference compatibility)
        feature_cols: List of feature column names (for inference compatibility)
