...
class DriftPredictor:
    ...
    def _load_model(self) -> nn.Module:
        ...
        # Extract feature_idx_map and scaler from checkpoint (BUG-011 fix)
        feature_idx_map = checkpoint.get("feature_idx_map")
        if feature_idx_map is None:
            feature_idx_map = checkpoint.get("model_config", {}).get("feature_idx_map")

        scaler = checkpoint.get("scaler")
        if scaler is None:
            scaler = checkpoint.get("model_config", {}).get("scaler")

        feature_cols = checkpoint.get("feature_cols")
        if feature_cols is None:
            feature_cols = checkpoint.get("model_config", {}).get("feature_cols")
        ...
        if model_type == "pinn":
            ...
            model = PINNDriftModel(
                ...
                feature_idx_map=feature_idx_map,  # Pass feature_idx_map to model
            )
        ...
        # Store scaler and feature_cols for use in prediction (BUG-011 fix)
        self.scaler = scaler
        self.feature_cols = feature_cols
        self.feature_idx_map = feature_idx_map

        logger.info(f"Loaded {model_type} model from {self.config.model_path}")
        if feature_idx_map is not None:
            logger.info(f"Loaded feature_idx_map with {len(feature_idx_map)} features")
        else:
            logger.warning(f"feature_idx_map not found in checkpoint {self.config.model_path} - may cause incorrect feature selection")
        if scaler is not None:
            logger.info("Loaded scaler for input normalization")
        else:
            logger.warning(f"scaler not found in checkpoint {self.config.model_path} - inputs will not be normalized as during training")
        return model
    ...
    def prepare_input_sequence(
        ...
        # Use restored feature_cols from checkpoint if available (BUG-011 fix)
        ...
        # Apply scaler if loaded (BUG-011 fix)
    ...
    def predict(
        ...
        # Drop the initial position/timestamp (index 0) so every array in the
        # result represents only the forecasted future steps and is the same
        # length as velocities_u/velocities_v. The starting point is already
        # available separately via init_lat/init_lon/init_time below.
        pred_times_future = pred_times[1:]
        pred_lats_future = pred_lats[1:]
        pred_lons_future = pred_lons[1:]

        result = {
            "timestamps": pred_times_future,
            "latitudes": pred_lats_future,
            "longitudes": pred_lons_future,
            "velocities_u": pred_u,
            "velocities_v": pred_v,
            "init_lat": init_lat,
            "init_lon": init_lon,
        ...
        if physics_pred is not None:
            ...
            result["physics_latitudes"] = phys_lats[0][1:]
            result["physics_longitudes"] = phys_lons[0][1:]
        ...
        if self.config.include_uncertainty and hasattr(self.model, 'ml_model'):
            ...
            # Drop the initial position (index 0) to match the forecasted steps
